...
![Version: 0.1.20](https://img.shields.io/badge/Version-0.1.20-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 42.20.2](https://img.shields.io/badge/AppVersion-42.20.2-informational?style=flat-square)
...
| discordBot.curator | object | `{"llm":{"allowPaid":false,"credentials":{"cerebras":{"existingSecret":"","existingSecretKey":"cerebras-api-key"},"gemini":{"existingSecret":"","existingSecretKey":"gemini-api-key"},"groq":{"existingSecret":"","existingSecretKey":"groq-api-key"},"openai":{"existingSecret":"","existingSecretKey":"openai-api-key"},"openrouter":{"existingSecret":"","existingSecretKey":"openrouter-api-key"}},"enabled":false,"globalCooldown":"5s","userCooldown":"30s"},"naturalChat":{"ambientReplyChance":0.25,"channelId":"","enabled":false,"globalCooldown":"5s","userCooldown":"20s"}}` | "The Curator" LLM chat feature (/curator command + optional natural chat trigger). Provider list/priority/model/allow_paid live in the discordbot_llm_providers Postgres table (requires discordBot.database.enabled above), NOT here -- this block only controls the bot's own feature flags and which provider credential env vars get injected. A DB row can only ever select a credential slot the bot's own Go code (and therefore this chart) already knows about -- see zomboid-exporter-ideas' curator-llm-provider-db-config.md. |
| discordBot.curator.llm.allowPaid | bool | `false` | Global safety gate: a provider row with allow_paid=true is only ever actually eligible when this is ALSO true -- a hot-editable Postgres row alone can never enable real spending. Leave false for a free-tier-only deployment. |
| discordBot.curator.llm.credentials | object | `{"cerebras":{"existingSecret":"","existingSecretKey":"cerebras-api-key"},"gemini":{"existingSecret":"","existingSecretKey":"gemini-api-key"},"groq":{"existingSecret":"","existingSecretKey":"groq-api-key"},"openai":{"existingSecret":"","existingSecretKey":"openai-api-key"},"openrouter":{"existingSecret":"","existingSecretKey":"openrouter-api-key"}}` | Provider API key secrets, one per credential_slot the bot's Go code recognizes (curator_llm.go's credentialSlots allowlist). Only set existingSecret for the slot(s) an actual discordbot_llm_providers row uses -- an unset one just means that slot can never resolve (fails closed). |
| discordBot.curator.llm.enabled | bool | `false` | Enable the LLM reply path (/curator open-ended questions + natural chat, when naturalChat.enabled above). Default OFF adds zero DB/network load. Canned Curator responses work regardless of this setting. |
| discordBot.curator.llm.globalCooldown | string | `"5s"` | Minimum time between ANY two LLM provider calls, across all users and both /curator and natural chat -- the real ceiling on provider quota consumption. |
| discordBot.curator.llm.userCooldown | string | `"30s"` | Minimum time between LLM provider calls for the same Discord user, shared by /curator and natural chat. Canned replies are exempt and never count against this budget. |
| discordBot.curator.naturalChat.ambientReplyChance | float | `0.25` | Probability (0-1) of replying to an ORDINARY statement that merely mentions "curator" -- a direct question/address always attempts a reply regardless of this. |
| discordBot.curator.naturalChat.channelId | string | `""` | Restrict the natural chat trigger to one channel ID. Empty means any channel the bot can see. |
| discordBot.curator.naturalChat.enabled | bool | `false` | React to the standalone word "curator" in ordinary channel messages, not just /curator. REQUIRES the Discord Message Content privileged intent to be enabled for this bot in the Discord Developer Portal, or Discord silently sends empty message content and this never triggers. Independent of curator.llm.enabled below -- canned Curator responses work through this trigger even with the LLM feature off. |
| discordBot.curator.naturalChat.globalCooldown | string | `"5s"` | Minimum time between ANY two natural-chat Curator replies, across all users. |
| discordBot.curator.naturalChat.userCooldown | string | `"20s"` | Minimum time between natural-chat Curator replies (canned or LLM) to the same Discord user. Conversational pacing, unrelated to LLM cost -- see curator.llm.userCooldown below for that. |
